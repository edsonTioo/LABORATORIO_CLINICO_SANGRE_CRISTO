...
namespace LaboratorioClinico.Controllers
{
    ...
    public class GraficosController : ControllerBase
    {
        ...
        public async Task<ActionResult<IEnumerable<object>>> GetIngresosPorTipoExamen()
        {
            try
            {
                // Obtenemos todos los detalles de orden con sus facturas y parámetros
                var detallesConPrecios = await _context.DetalleOrdens
                    .Include(d => d.IdtipoExamenNavigation)
                    .Include(d => d.DetalleFacturas)
                    .Select(d => new
                    {
                        ExamenId = d.IdtipoExamen,
                        ExamenNombre = d.IdtipoExamenNavigation.NombreExamen,
                        PrecioTotal = d.DetalleFacturas.Sum(df => df.Precio) // Sumamos precios de las facturas
                    })
                    .ToListAsync();

                // Agrupamos por examen y calculamos totales
                var resultados = detallesConPrecios
                    .GroupBy(d => new { d.ExamenId, d.ExamenNombre })
                    .Select(g => new
                    {
                        Examen = g.Key.ExamenNombre,
                        Cantidad = g.Count(),
                        TotalIngresos = g.Sum(x => x.PrecioTotal)
                    })
    ...
    try
    {
        ...
        var examenes = await _context.DetalleOrdens
            ...
            .Select(g => new
            {
                Examen = g.Key,
                Cantidad = g.Count(),
                TotalIngresos = g.Sum(d => d.IdtipoExamenNavigation.Parametros.Sum(p => p.Precio)),
