 ...
 function GestionOrdenesScreen({ navigation }) {
  ...
  const validarCampos = () => {
    const newErrors = {
      ...
      precioExamen: !nuevoDetalle.precioExamen || nuevoDetalle.precioExamen <= 0
    };
    setErrors(newErrors);
    return !Object.values(newErrors).some(error => error);
  };

  const validarExamenDuplicado = () => {
    if (!selectedOrden || !nuevoDetalle.idTipoExamen) return false;

    // Acceso seguro a los detalles, considerando diferentes estructuras posibles
    const detalles = Array.isArray(selectedOrden.detalles)
      ? selectedOrden.detalles
      : (selectedOrden.detalles?.$values || []);

    // Verificar si ya existe un detalle con el mismo idTipoExamen
    return detalles.some(detalle => detalle.idTipoExamen === nuevoDetalle.idTipoExamen);
  };

  const agregarDetalle = async () => {
    if (!validarCampos()) return;

    if (validarExamenDuplicado()) {
      const examenDuplicado = examenes.find(e => e.idTipoExamen === nuevoDetalle.idTipoExamen);
      Toast.show({
        type: 'error',
        text1: 'Error',
        text2: `El examen "${examenDuplicado?.nombreExamen || 'seleccionado'}" ya está en la orden`
      });
      return;
    }
    ...
    try {
      const response = await fetch(`${API_BASE}/agregar-detalle/${selectedOrden.idOrden}`, {
        ...
        body: JSON.stringify({
          idTipoExamen: nuevoDetalle.idTipoExamen,
          idMuestra: nuevoDetalle.idMuestra,
          precioExamen: nuevoDetalle.precioExamen
        })
      });
  ...
  const getTotalOrden = (detalles) => {
    if (!detalles || !detalles.$values) return 0;
    return detalles.$values.reduce((sum, detalle) => sum + (detalle.precioExamen || 0), 0);
  };
              ...
              <Card.Content>
                    ...
                    right={() => (
                      <View style={styles.detailRight}>
                        <Text style={styles.priceText}>
                          ${detalle.precioExamen.toFixed(2)}
                        </Text>
                ...
                <View style={styles.totalContainer}>
                  <Text style={styles.totalText}>
                    Total: ${getTotalOrden(orden.detalles).toFixed(2)}
                  </Text>
                </View>
              </Card.Content>
...
<TextInput
  label="Precio"
  value={nuevoDetalle.precioExamen.toString()}
  onChangeText={text => {
    ...
    setErrors({
      ...errors,
      precioExamen: precio <= 0
    });
  }}
  keyboardType="numeric"
  style={[styles.input, errors.precioExamen && styles.errorInput]}
  mode="outlined"
  error={errors.precioExamen}
  theme={{ colors: { text: 'black', placeholder: 'black', primary: 'black' } }}
  textColor="black"
  selectionColor="black"
/>

              <HelperText type="error" visible={errors.precioExamen}>
                {nuevoDetalle.precioExamen <= 0 ? "El precio debe ser mayor a 0" : "Este campo es obligatorio"}
              </HelperText>
            </Card.Content>
...
const styles = StyleSheet.create({
  ...
  textItem: {
    flex: 1,
    fontSize: 16, color: 'black', // <- Aquí el color debe ser negro
  },
  icon: {
    marginRight: 5,
  },
